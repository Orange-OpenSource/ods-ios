...
struct AppsPlusToMoreAppsMapper {
    ...
    func appsSections(from appsList: AppsPlusListDTO) -> [MoreAppsListSection] {
        appsList.sections.map { appsSection(from: $0) }
    }

    func appsDetails(from appsList: AppsPlusListDTO) -> [MoreAppDetails] {
        appsList.apps.map { appDetails(from: $0) }
    }

    func appsSection(from section: AppsPlusSectionDTO) -> MoreAppsListSection {
        MoreAppsListSection(description: section.description,
                        apps: section.apps.map { appDetails(from: $0) })
    }

    func appDetails(from details: AppsPlusAppDetailsDTO) -> MoreAppDetails {
        MoreAppDetails(title: details.title,
