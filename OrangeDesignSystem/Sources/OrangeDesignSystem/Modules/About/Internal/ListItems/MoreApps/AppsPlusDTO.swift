...
struct AppsPlusRepository: MoreAppsRepositoryProtocol {
    ...
    /// Using the some `URL` and  `URLSessionConfiguration` gets some `AppsPlusDTO` object from the _Apps Plus_ backend, parse it
    ...
    private func cache(data payload: Data) {
        do {
            if let localCacheLocation = localCacheLocation, FileManager.default.fileExists(atPath: localCacheLocation.path) {
                ...
                ODSLogger.debug("JSON file for AppsPlus has been saved at '\(localCacheLocation.path)'")
            } else {
